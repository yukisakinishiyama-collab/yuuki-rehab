...
【断片を臨床項目に入れない（最重要）】
precautions（注意・禁忌）、advanceCriteria（進行基準）、goals、redFlags には、
文として完全に読み取れたものだけを入れてください。
- 端が切れている、途中で欠けている、意味の通らない断片（例: "伸L" "se" "D 1-7" "トレッチ 内"）は
  これらの欄に入れず、unreadable にだけ書く
- どのPhaseに属するか確信が持てない記述も、これらの欄に入れず unreadable に書く
- 荷重制限・可動域制限は特に重要ですが、断片のまま載せるほうが危険です。
  完全に読めないなら「左端の制限欄が切れており読み取れない」と unreadable に書いてください
...
- **行ごとに印の範囲は異なります。同じPhaseだからといって全行に同じ範囲を当てはめないでください**
- 用紙が傾いている等で、行の名称と印の行の対応に確信が持てない場合は、
  dose を空文字にしたうえで unreadable に「傾きのため行と印の対応が不確実」と書く。
  推測した週数を書くより、空のほうが安全です
- 色や区切りで示されたPhaseの見出しは title に使う。
  セクション見出し（例: セクション1 (SEC1)）は exercises に単独で入れず、
  配下の運動名の先頭に「[SEC1] 」のように付けて所属が分かるようにする
