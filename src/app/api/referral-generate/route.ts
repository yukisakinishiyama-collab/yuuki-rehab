...
以下のJSON形式で出力してください。マークダウン・余分なテキスト不要：
{
  ...
  "recommendedFrequency": "治療頻度の推奨（急性期・回復期・維持期ごとの来院頻度の目安を1〜2文で。例：急性期は週3回、症状軽減後は週1〜2回を推奨します。）"
}`
    } else {
...
以下のJSON形式で出力してください。マークダウン・余分なテキスト不要：
{
  ...
  "recommendedFrequency": "治療頻度の推奨（現在の施術頻度と今後の方針を1〜2文で。例：現在週2回の施術を継続中、術後の状態に応じて頻度を調整予定です。）"
}`
    }
