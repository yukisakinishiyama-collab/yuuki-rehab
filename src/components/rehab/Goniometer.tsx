...
export default function Goniometer() {
  ...
  const handleOrientation = useCallback((e: DeviceOrientationEvent) => {
    ...
    // ── 傾斜角の算出 ──
    // beta / gamma をそのまま角度として使うと、垂直付近で数値が壊れる。
    //
    // 原因は「同じ姿勢に2通りの (beta, gamma) 表現がある」こと。
    // gamma は仕様上 ±90° に収める必要があるため、端末が垂直に近づくと
    // 実機は (beta, gamma) と (180-beta, gamma-180) の表現を行き来する。
    //   真のピッチ 95° → beta が 95° と 85° の間で飛ぶ
    //   真のピッチ120° → beta が 120° と 60° の間で飛ぶ
    // 「90°を境に数値が戻る」現象はこれが原因。
    // gamma 軸も同様に ±90° を超えると折り返す（真のロール120° → gamma -60°）。
    //
    // 一方、端末座標系での重力（＝「上」）ベクトルは
    // どちらの表現から計算しても完全に同一になる。
    // そこで重力ベクトルを復元してから傾斜角を組み立てる。
    const gx = -Math.cos(bRad) * Math.sin(gRad)
    const gy = Math.sin(bRad)
    const gz = Math.cos(bRad) * Math.cos(gRad)

    // 水平面への投影の大きさ。垂直付近で 0 に近づく
    const horiz = Math.hypot(gx, gz)

    let incl: number
    if (m.axis === 'beta') {
      // 長軸の傾き。atan2 で組むことで垂直付近でも数値が安定し、
      // 端末のねじれ（gamma）に影響されない。
      const phi = Math.atan2(gy, horiz) / DEG          // -90〜90
      // 垂直を越えた側へ展開して -180〜180 にする。
      // 折り返し点では両式とも同じ値になるため不連続は生じない。
      incl = gz >= 0 ? phi : (gy >= 0 ? 180 - phi : -180 - phi)
    } else {
      // 短軸まわりの回転角。±180° まで連続。
      // 端末がほぼ垂直だと水平成分が消えてこの角度は定義できないため、
      // その間はサンプルを捨てて直前の値を保持する。
      if (horiz < 0.1) return
