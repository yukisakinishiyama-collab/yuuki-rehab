...
interface DrillsBrowserProps {
  /** 初期表示で絞り込む疾患スラッグ（例: 患者の診断名から推定した値） */
  initialDiseaseSlug?: string
  /** 症状詳細ページへのリンクを表示するか（患者ケース内での埋め込み時はfalse推奨） */
  showSymptomLink?: boolean
}

export default function DrillsBrowser({ initialDiseaseSlug, showSymptomLink = true }: DrillsBrowserProps) {
  const [disease, setDisease] = useState(initialDiseaseSlug ?? 'all')
